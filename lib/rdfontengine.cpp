...
// QFont copies share their data until modified. Reuse identical pixel fonts.
static QFont SharedFont(const QString &family,int size,int weight)
{
  QFont font(family,size,weight);
  font.setPixelSize(size);
  static QHash<QFont,QFont> fonts;
  static QMutex mutex;
  QMutexLocker locker(&mutex);
  if(!fonts.contains(font)) {
    fonts.insert(font,font);
  }
...
void RDFontEngine::MakeFonts(const QFont &default_font)
{
  ...
  */
  ...
  // Generate Fonts
  //
  font_button_font=SharedFont(family,button_size,QFont::Bold);
  font_button_font_metrics=new QFontMetrics(font_button_font);

  font_huge_button_font=SharedFont(family,button_size+24,QFont::DemiBold);
  font_huge_button_font_metrics=new QFontMetrics(font_huge_button_font);

  font_big_button_font=SharedFont(family,button_size+4,QFont::DemiBold);
  font_big_button_font_metrics=new QFontMetrics(font_big_button_font);

  font_sub_button_font=SharedFont(family,button_size-2,QFont::Normal);
  font_sub_button_font_metrics=new QFontMetrics(font_sub_button_font);

  font_section_label_font=SharedFont(family,label_size+2,QFont::Bold);
  font_section_label_font_metrics=new QFontMetrics(font_section_label_font);

  font_big_label_font=SharedFont(family,label_size+4,QFont::Bold);
  font_big_label_font_metrics=new QFontMetrics(font_big_label_font);

  font_label_font=SharedFont(family,label_size,QFont::Bold);
  font_label_font_metrics=new QFontMetrics(font_label_font);

  font_sub_label_font=SharedFont(family,label_size,QFont::Normal);
  font_sub_label_font_metrics=new QFontMetrics(font_sub_label_font);

  font_progress_font=SharedFont(family,label_size+4,QFont::Bold);
  font_progress_font_metrics=new QFontMetrics(font_progress_font);

  font_banner_font=SharedFont(family,26,QFont::Normal);
  font_banner_font_metrics=new QFontMetrics(font_banner_font);

  font_timer_font=SharedFont(family,20,QFont::Normal);
  font_timer_font_metrics=new QFontMetrics(font_timer_font);

  font_small_timer_font=SharedFont(family,default_size+2,QFont::Normal);
  font_small_timer_font_metrics=new QFontMetrics(font_small_timer_font);

  font_default_font=SharedFont(family,default_size,QFont::Normal);
